...
import {
  ...
  requireArg,
  saveBlobToFile,
} from "file:///C:/Users/ks343/.codex/plugins/cache/openai-primary-runtime/presentations/26.630.12135/skills/presentations/container_tools/artifact_tool_utils.mjs";
...
const unzipCommand =
  "C:\\ds-fashion-new\\work\\presentations\\aicte-eduskills-php-internship\\tmp\\bin\\unzip.cmd";
...
function zipNames(pptxPath) {
  return String(runCapture(unzipCommand, ["-Z1", pptxPath], { encoding: "utf8" }))
    .split(/\r?\n/)
    .filter(Boolean);
}

function readZipText(pptxPath, entryName) {
  return Buffer.from(runCapture(unzipCommand, ["-p", pptxPath, entryName])).toString("utf8");
}

async function copyZipEntry(pptxPath, entryName, targetPath) {
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, Buffer.from(runCapture(unzipCommand, ["-p", pptxPath, entryName])));
}
